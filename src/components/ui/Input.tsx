...
export const Input: React.FC<InputProps> = ({ label, name, error, icon, className, labelClassName, wrapperClassName, ...props }) => {
...
export const Textarea: React.FC<TextareaProps> = ({ label, name, error, className, labelClassName, wrapperClassName, ...props }) => {
  const hasError = Boolean(error);
  return (
    <div className={`w-full ${wrapperClassName || ''}`}>
      {label && (
