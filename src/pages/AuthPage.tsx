...
import { AppLogoIcon, cn } from '../constants.tsx'; 
...
const pageVariants: Variants = { initial: { opacity: 0, y: 20 }, in: { opacity: 1, y: 0, transition: { duration: 0.5, ease: "circOut" as const } }, out: { opacity: 0, y: -20, transition: { duration: 0.3, ease: "circIn" as const } } };
const cardVariants: Variants = { initial: { opacity: 0, scale: 0.95 }, in: { opacity: 1, scale: 1, transition: { duration: 0.4, delay: 0.2, ease: "circOut" as const } }, out: { opacity: 0, scale: 0.95, transition: { duration: 0.2, ease: "circIn" as const } } };
...
type ViewType = 'login' | 'register' | 'forgotPassword' | 'emailVerification' | 'updatePassword'; // Adicionado 'updatePassword'
...
const AuthPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Check for update_password in hash for direct navigation or after Supabase PASSWORD_RECOVERY event
  ...
  useEffect(() => {
    if (authEvent === 'PASSWORD_RECOVERY' && location.hash.includes('type=update_password')) {
      setCurrentView('updatePassword');
      setAuthEvent(null); // Consumir o evento
    }
  ...
  return (
    <motion.div key="authPageContainer" className="min-h-screen auth-page-reimagined-theme flex flex-col justify-center py-12 sm:px-6 lg:px-8 overflow-hidden relative" variants={pageVariants} initial="initial" animate="in" exit="out" >
      <div className="aurora-effect gold-aurora"></div> <div className="aurora-effect green-aurora"></div>
      <motion.div className="sm:mx-auto sm:w-full sm:max-w-md text-center z-10" initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0, transition: { duration: 0.5, delay: 0.1, ease: "circOut" as const } }} >
      ...
      <motion.div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md z-10" variants={cardVariants} initial="initial" animate="in" exit="out" >
        <div className="bg-auth-card-bg border border-auth-card-border shadow-2xl rounded-2xl py-10 px-6 sm:px-10 backdrop-filter backdrop-blur-xl">
          <AnimatePresence mode="wait">
            <motion.div key={currentView} initial={{ opacity: 0, x: currentView === 'login' ? -30 : 30 }} animate={{ opacity: 1, x: 0, transition: { duration: 0.4, ease: "circOut" as const } }} exit={{ opacity: 0, x: currentView === 'login' ? 30 : -30, transition: { duration: 0.2, ease: "circIn" as const } }} >
              {currentView === 'emailVerification' ? (
                ...
                <form ref={formRef} onSubmit={handleSubmit} className="space-y-6">
                  {currentView === 'register' && ( <div> <label htmlFor="fullName" className="block text-sm font-semibold text-auth-text-secondary mb-1.5"> Nome Completo </label> <div className="relative"> <div className={ICON_WRAPPER_CLASSES}> <UserIcon className="h-5 w-5"/> </div> <input id="fullName" name="fullName" type="text" autoComplete="name" required value={formState.fullName} onChange={(e) => updateFormField('fullName', e.target.value)} className={cn(COMMON_INPUT_CLASSES, 'pl-12', formErrors.fullName ? 'border-red-500' : '')} disabled={formLoading} placeholder="Digite seu nome completo" /> </div> {formErrors.fullName && ( <p className="mt-1 text-sm text-red-400">{formErrors.fullName}</p> )} </div> )}
                  {currentView !== 'updatePassword' && ( <div> <label htmlFor="email" className="block text-sm font-semibold text-auth-text-secondary mb-1.5"> Endereço de e-mail </label> <div className="relative"> <div className={ICON_WRAPPER_CLASSES}> <EmailIcon className="h-5 w-5"/> </div> <input id="email" name="email" type="email" autoComplete="email" required value={formState.email} onChange={(e) => updateFormField('email', e.target.value)} className={cn(COMMON_INPUT_CLASSES, 'pl-12', formErrors.email ? 'border-red-500' : '')} disabled={formLoading} placeholder="seu@email.com" /> </div> {formErrors.email && ( <p className="mt-1 text-sm text-red-400">{formErrors.email}</p> )} </div> )}
                  {(currentView === 'login' || currentView === 'register' || currentView === 'updatePassword') && ( <div> <label htmlFor="password" className="block text-sm font-semibold text-auth-text-secondary mb-1.5"> {currentView === 'updatePassword' ? 'Nova Senha' : 'Senha'} </label> <div className="relative"> <div className={ICON_WRAPPER_CLASSES}> <LockClosedIcon className="h-5 w-5"/> </div> <input id="password" name="password" type="password" autoComplete={currentView === 'register' || currentView === 'updatePassword' ? "new-password" : "current-password"} required value={formState.password} onChange={(e) => updateFormField('password', e.target.value)} className={cn(COMMON_INPUT_CLASSES, 'pl-12', formErrors.password ? 'border-red-500' : '')} disabled={formLoading} placeholder="Digite sua senha" /> </div> {formErrors.password && ( <p className="mt-1 text-sm text-red-400">{formErrors.password}</p> )} </div> )}
                  {(currentView === 'register' || currentView === 'updatePassword') && ( <div> <label htmlFor="confirmPassword" className="block text-sm font-semibold text-auth-text-secondary mb-1.5"> Confirmar {currentView === 'updatePassword' ? 'Nova Senha' : 'Senha'} </label> <div className="relative"> <div className={ICON_WRAPPER_CLASSES}> <LockClosedIcon className="h-5 w-5"/> </div> <input id="confirmPassword" name="confirmPassword" type="password" autoComplete="new-password" required value={formState.confirmPassword} onChange={(e) => updateFormField('confirmPassword', e.target.value)} className={cn(COMMON_INPUT_CLASSES, 'pl-12', formErrors.confirmPassword ? 'border-red-500' : '')} disabled={formLoading} placeholder="Confirme sua senha" /> </div> {formErrors.confirmPassword && ( <p className="mt-1 text-sm text-red-400">{formErrors.confirmPassword}</p> )} </div> )}
                  {formErrors.general && ( <p className="text-sm text-red-400 text-center p-3 bg-red-900/30 rounded-lg border border-red-700/50"> {formErrors.general} </p> )}
                  {successMessage && (currentView === 'forgotPassword' || currentView === 'updatePassword') && ( <p className="text-sm text-green-400 text-center p-3 bg-green-900/30 rounded-lg border border-green-700/50"> {successMessage} </p> )}
                  <div className="pt-2"> <AuthButton type="submit" isLoading={formLoading} disabled={!isFormSubmittable || formLoading} > {currentView === 'register' ? 'Criar Conta' : currentView === 'login' ? 'Entrar' : currentView === 'updatePassword' ? 'Salvar Nova Senha' : 'Enviar Link'} </AuthButton> </div>
                  {currentView === 'login' && ( <div className="text-sm text-center"> <AuthButton variant="link" onClick={() => handleViewChange('forgotPassword')}> Esqueceu sua senha? </AuthButton> </div> )}
                </form>
              )}
            </motion.div>
          </AnimatePresence>
      ...
      <p className="text-xs text-auth-text-secondary/50 text-center mt-10 z-10"> &copy; {new Date().getFullYear()} 1Checkout. Todos os direitos reservados. </p>
    </motion.div>
  );
};
