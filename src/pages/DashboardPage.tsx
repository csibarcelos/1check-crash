...
import { Select as RadixSelect } from '@/components/ui/Select'; // Renomeado para evitar conflito
import { ResponsiveContainer, LineChart, Line, XAxis, YAxis, CartesianGrid, Tooltip, Legend } from 'recharts';
import { DayPicker, DateRange } from 'react-day-picker';
import { format as formatDate } from 'date-fns';
import { ptBR } from 'date-fns/locale/pt-BR'; // Corrected import path
...
const DashboardPage: React.FC = () => {
  ...
  const fetchInitialData = useCallback(async () => {
    if (authLoading || !accessToken) {
        setIsLoading(false);
        if (!accessToken) setError("Autenticação necessária.");
        return;
    }
  ...
  useEffect(() => {
    if (!isLoading && allSales.length >= 0 && accessToken) { 
      try {
        ...
        const transformedSalesTrend = data.salesTrend.map(item => ({
    ...
    } else if (!isLoading && allSales.length === 0 && accessToken) {
        setDashboardData({
  ...
  }, [allSales, allProducts, allCustomers, dateRangeFilter, productFilter, customDateRange, isLoading, accessToken]);

  const handleDateRangeFilterChange = (value: string) => {
    setDateRangeFilter(value);
    if (value !== 'custom') {
      setCustomDateRange(undefined); // Limpa o range personalizado se outra opção for selecionada
    } else {
      setIsCustomRangePopoverOpen(true); // Abre o popover se 'custom' for selecionado
    }
  };
  ...
  const handleCustomDateRangeSelect = (range: DateRange | undefined) => {
    ...
    } else if (range?.from && !range.to) { // Apenas uma data selecionada, mantém o from
  ...
  if (authLoading || (isLoading && !dashboardData && !error)) { 
    return (
              ...
              <PopoverPrimitive.Portal>
                <PopoverPrimitive.Content
                    ...
                    className={cn(
                        ...
                        "bg-bg-surface bg-opacity-90 backdrop-blur-lg shadow-2xl p-0", // Removido padding p-2 daqui
                ...
                >
                    <DayPicker
                        ...
                        className="p-3" // Adicionado padding interno ao DayPicker
                        classNames={{
        ...
        {rechartsSalesData && rechartsSalesData.length > 0 ? (
          <div className="h-80 bg-bg-surface p-4 rounded-lg border border-border-subtle backdrop-blur-sm">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={rechartsSalesData} margin={{ top: 5, right: 20, left: -20, bottom: 5 }}>
                ...
                <Tooltip
                  ...
                  labelStyle={{ color: 'var(--color-accent-gold)', fontWeight: 'bold', marginBottom: '4px', borderBottom: '1px solid var(--color-border-subtle)', paddingBottom: '4px' }}
                  formatter={(value: number) => [`R$ ${value.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`, "Vendas"]}
                  cursor={{ stroke: 'var(--color-accent-blue-neon)', strokeWidth: 1, strokeDasharray: '3 3' }}
                />
                <Legend 
                  wrapperStyle={{ color: 'var(--color-text-muted)', paddingTop: '10px', fontSize: '12px' }} 
                ...
                <Line 
                  ...
                  strokeWidth={2.5} 
                  dot={{ r: 4, fill: 'var(--color-accent-blue-neon)', stroke: 'var(--color-bg-main)', strokeWidth: 2 }} 
                  activeDot={{ r: 7, fill: 'var(--color-accent-blue-neon)', stroke: 'var(--color-bg-main)', strokeWidth: 2, filter: 'drop-shadow(0 0 5px var(--color-accent-blue-neon))' }} 
                  name="Vendas"
                />
              </LineChart>
            </ResponsiveContainer>
                ...
                <div className="flex justify-between items-center">
                  <div>
                    ...
                    <p className="text-xs text-text-muted">
                      {sale.products.map(p => p.name).join(', ')} - {new Date(sale.createdAt).toLocaleString('pt-BR', { day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit' })}
                    </p>
                  </div>
