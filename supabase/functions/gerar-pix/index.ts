...
// [ As interfaces (PixGenerationRequestPayload, etc.) permanecem as mesmas ]
// ... (vou omitir as interfaces para encurtar, elas não mudam)
...
serve(async (req: Request) => {
  ...
  try {
    ...
    // ... (lógica de criação de venda, chamada à PushInPay e verificação permanecem as mesmas)
    ...
    // 5. ETAPA DE VERIFICAÇÃO CRÍTICA
    ...
    // 6. DISPARAR EVENTO DE VENDA PENDENTE PARA UTMIFY (não bloqueante)
    try {
      ...
      const utmifyPayloadForWaiting: UtmifyOrderPayloadForWaitingPayment = {
        ...
        // ==================================================================
        // CORREÇÃO: Adicionando o objeto 'commission' que estava faltando.
        // ==================================================================
        commission: {
            totalPriceInCents: payload.value,
            gatewayFeeInCents: totalPlatformShareForPushInPay, // Usamos a comissão da plataforma como uma aproximação da taxa.
            userCommissionInCents: payload.value - totalPlatformShareForPushInPay, // Valor total menos a taxa.
            currency: "BRL"
        },
