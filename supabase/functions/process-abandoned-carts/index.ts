  ...
  try {
    ...
    for (const userSetting of usersWithSettings as AppSettingsDbRow[]) {
      ...
      try {
        ...
        console.log(`[process-abandoned-carts] Processando para usuário ${platformUserId} com delay de ${config.delayHours}h.`);

        // 2. Buscar carrinhos abandonados elegíveis para este usuário
        const delayInMilliseconds = config.delayHours * 60 * 60 * 1000;
